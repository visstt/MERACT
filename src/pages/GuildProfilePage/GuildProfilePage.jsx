...
export const GuildProfilePage = () => {
  ...
  useEffect(() => {
    if (inviteOpen && allUsers.length === 0) {
      api.get('/user/all-users-for-guild').then(res => setAllUsers(res.data)).catch(() => setAllUsers([]));
    }
  ...
  const handleInvite = async () => {
    ...
    try {
      await api.post(`/guild/invite-user?userId=${inviteValue}&guildId=${id}`);
      setInviteOpen(false);
      setInviteValue('');
      const res = await api.get(`/guild/${id}`);
      setMembers(res.data.members || []);
  ...
  const handleKick = async (userId) => {
    ...
    try {
      await api.post(`/guild/kick-out-user?userId=${userId}&guildId=${id}`);
      const res = await api.get(`/guild/${id}`);
      setMembers(res.data.members || []);
    } catch {
  ...
  return (
    <div className={styles.guildProfilePage}>
      <div className={styles.header}>
        <img className={styles.logo} src={guild.logoFileName ? getImageUrl('guild', guild.logoFileName) : ''} alt={guild.name} />
      ...
      <div className={styles.membersBlock}>
        <div className={styles.membersHeader}>
          ...
          <button className={styles.inviteButton} onClick={() => setInviteOpen(true)}>Invite user</button>
        </div>
        <div className={styles.membersList}>
          {members.length === 0 && <div className={styles.empty}>No members</div>}
          {members.map(user => (
            <div key={user.id} className={styles.memberCard}>
              <div className={styles.memberAvatar}>{user.login ? user.login[0].toUpperCase() : user.email[0].toUpperCase()}</div>
              <div className={styles.memberInfo}>
                <div className={styles.memberName}>{user.login || user.email}</div>
                <div className={styles.memberEmail}>{user.email}</div>
              </div>
              <button className={styles.kickButton} onClick={() => handleKick(user.id)} title="Kick">Kick</button>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className={styles.inviteModalOverlay}>
          <div className={styles.inviteModal}>
            ...
            <div className={styles.inviteActions}>
              <button className={styles.cancelButton} onClick={() => setInviteOpen(false)}>Cancel</button>
              <button className={styles.inviteButton} onClick={handleInvite} disabled={inviteLoading || !inviteValue}>
